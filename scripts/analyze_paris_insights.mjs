...
const data = JSON.parse(await fs.readFile(
  path.join(ROOT, 'data/processed/studios_consolidated_boutique_v2.json'),
  'utf8'
));

console.log('='.repeat(80));
console.log('PARIS BOUTIQUE FITNESS - KEY INSIGHTS ANALYSIS');
console.log('='.repeat(80));
console.log('');
...
const withArr = data.filter(s => s.arrondissement);
console.log(`Studios with arrondissement: ${withArr.length} (${(withArr.length / data.length * 100).toFixed(1)}%)`);
console.log('');
...
// 4. REFORMER PILATES BOOM
console.log('4. REFORMER PILATES ANALYSIS');
console.log('-'.repeat(80));
const reformerStudios = data.filter(s => 
  (s.categories || []).some(cat => 
    cat.includes('reformer') || cat === 'reformer-pilates' || cat === 'pilates'
  )
);

const reformerWithDates = reformerStudios.filter(s => s.estimated_opening_date);
const reformerYears = {};
reformerWithDates.forEach(s => {
  const year = new Date(s.estimated_opening_date).getFullYear();
  if (year >= 2015) {
    reformerYears[year] = (reformerYears[year] || 0) + 1;
  }
});

console.log(`Total studios offering Pilates: ${reformerStudios.length} (${(reformerStudios.length / data.length * 100).toFixed(1)}%)`);
console.log('\n  Pilates openings by year (2015+):');
Object.entries(reformerYears)
  .map(([year, count]) => ({ year: parseInt(year), count }))
  .sort((a, b) => a.year - b.year)
  .forEach(({ year, count }) => {
...
console.log('5. SPATIAL DISTRIBUTION (TOP 15 ARRONDISSEMENTS)');
console.log('-'.repeat(80));
const arrCounts = {};
data.forEach(s => {
  const arr = s.arrondissement;
  if (arr) {
    arrCounts[arr] = (arrCounts[arr] || 0) + 1;
  }
});

const topArr = Object.entries(arrCounts)
  .map(([arr, count]) => ({ arr, count, pct: (count / data.length * 100).toFixed(1) }))
  .sort((a, b) => b.count - a.count)
  .slice(0, 15);

topArr.forEach((a, i) => {
  const bar = '█'.repeat(Math.floor(a.count / 5));
  console.log(`  ${(i + 1).toString().padStart(2)}. ${a.arr.padEnd(12)} ${a.count.toString().padStart(3)} studios (${a.pct.padStart(5)}%) ${bar}`);
});
console.log('');
...
// 8. MODALITY COMBINATIONS
console.log('8. POPULAR MODALITY COMBINATIONS');
console.log('-'.repeat(80));
...
const totalPilates = reformerStudios.length;
const pilatesPct = (totalPilates / data.length * 100).toFixed(1);
const recentPct = (recentStudios.length / data.length * 100).toFixed(1);
const topArrName = topArr[0]?.arr || 'N/A';
const topArrCount = topArr[0]?.count || 0;
const topModality = topModalities[0]?.modality || 'N/A';
const topModalityPct = topModalities[0]?.pct || '0';

console.log(`1. MARKET SIZE: ${data.length} boutique studios across Paris`);
console.log(`2. PILATES DOMINANCE: ${pilatesPct}% of studios offer Pilates (${totalPilates} studios)`);
...
console.log(`5. SPATIAL CONCENTRATION: ${topArrName} leads with ${topArrCount} studios`);
...
// 10. COMPARATIVE INSIGHTS (if London data available)
console.log('='.repeat(80));
...
console.log('');
console.log('3. ARRONDISSEMENT MODALITY HEATMAP');
console.log('   - Which modalities dominate in which arrondissements');
