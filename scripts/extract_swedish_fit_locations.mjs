...
async function main() {
  ...
  const swedishFitLocations = studios.filter(s => s.name === 'Swedish Fit');
  
  if (swedishFitLocations.length === 0) {
    console.log('Swedish Fit: Not found\n');
    return;
  }
  
  console.log(`\n${'='.repeat(80)}`);
  console.log(`Swedish Fit (${swedishFitLocations.length} locations)`);
  console.log('='.repeat(80));
  
  swedishFitLocations.forEach((loc, idx) => {
